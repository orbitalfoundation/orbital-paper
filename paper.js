	...
	if(invisible) {
		...
		if(paper._dom) {
			if(parent_paper && parent_paper._dom) {
				console.log("paper: dom already exists - rebinding",entity.uuid)
				parent_paper._dom.appendChild(paper._dom)
			} else {
				console.log("paper: dom already exists - rebinding to root",entity.uuid)
				document.body.appendChild(paper._dom)
			}
			return
		}
	}
	//console.log("paper - reacting for the first time",entity.uuid)
...
// @todo should be hiding and showing to the SAME POINT as before not inserting at the end
	...
	// node may specify its own parent by name - this is useful for updating fragments instead of entire hierarchies
	if(paper.parent) {
		const parent = document.getElementById(paper.parent)
		if(parent) {
			console.log("********* paper found a parent for ",paper.uuid,paper.parent)
			parent.appendChild(paper._dom)
		}
	}

	// can fallback to using a specified parent if any
	else if(parent_paper && parent_paper._dom) {
		parent_paper._dom.appendChild(paper._dom)
	} else {
		document.body.appendChild(paper._dom)
	}
...
function _paper_evaluate_entity_against_path(sys,path,entity,parent=null) {
	...
	// may attach to dom or mark as invisible - entity may specify its own parent!
